...
const ReconciliationDetailView = ({ reconciliation, onBack, incomingInvoices, outgoingInvoices }: { reconciliation: Reconciliation; onBack: () => void; incomingInvoices: IncomingInvoice[]; outgoingInvoices: OutgoingInvoice[]; }) => {
    ...
    const handleDownloadPdf = async () => {
        if (customer && relatedInvoices.incoming && relatedInvoices.outgoing) {
            const result = await downloadReconciliationAsPdf(reconciliation, customer, [relatedInvoices.incoming, relatedInvoices.outgoing] as any, t);
            if (result.success) {
                ...
                <div className="md:col-span-2 space-y-4">
                    <div className="p-4 bg-cnk-panel-light rounded-lg shadow-sm border">
                        ...
                        <div className="grid grid-cols-2 gap-4 text-sm">
                            ...
                            <p><strong>{t('amount')}:</strong> {reconciliation.amount.toLocaleString('tr-TR', { style: 'currency', currency: 'TRY' })}</p>
                        </div>
                    </div>
                    <div className="p-4 bg-cnk-panel-light rounded-lg shadow-sm border">
...
const ReconciliationPage = () => {
    ...
    const invoiceColumns = (type: 'incoming' | 'outgoing') => [
        { header: t('invoiceDate'), accessor: (item: any) => new Date(item.tarih).toLocaleDateString() },
        ...
        { header: t('totalAmount'), accessor: (item: any) => item.tutar.toLocaleString('tr-TR', { style: 'currency', currency: 'TRY' }) },
    ];

...
export const ErpProvider = ({ children }: ErpProviderProps) => {
    ...
    const syncIncomingInvoices = async (): Promise<SyncResult> => {
        ...
        const settings = erpSettings || await db.erpSettings.get('default');
        if (settings) {
            await updateErpSettings({ ...settings, lastSyncIncomingInvoices: new Date().toISOString() });
        }
    ...
    const syncOutgoingInvoices = async (): Promise<SyncResult> => {
        ...
        const settings = erpSettings || await db.erpSettings.get('default');
        if (settings) {
            await updateErpSettings({ ...settings, lastSyncOutgoingInvoices: new Date().toISOString() });
        }
    ...
    const syncCustomers = async (): Promise<SyncResult> => {
        ...
        const settings = erpSettings || await db.erpSettings.get('default');
        if (settings) {
            await updateErpSettings({ ...settings, lastSyncCustomers: new Date().toISOString() });
        }
    ...
    const syncInvoices = async (): Promise<SyncResult> => {
        ...
        const settings = erpSettings || await db.erpSettings.get('default');
        if (settings) {
            await updateErpSettings({ ...settings, lastSyncInvoices: new Date().toISOString() });
        }
    ...
    const syncOffers = async (): Promise<SyncResult> => {
        ...
        const settings = erpSettings || await db.erpSettings.get('default');
        if (settings) {
            await updateErpSettings({ ...settings, lastSyncOffers: new Date().toISOString() });
        }
